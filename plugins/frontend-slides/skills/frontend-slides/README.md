...
- **Single File, No Build Step** — Self-contained HTML styled with Tailwind CSS utility classes (via CDN script). No npm, no build tools.
- **Tailwind Styling Conventions** — All layout/type/color styling lives in utility classes; raw CSS is confined to three labeled blocks (stage mechanics, design tokens, choreography), so everything stays traceable.
