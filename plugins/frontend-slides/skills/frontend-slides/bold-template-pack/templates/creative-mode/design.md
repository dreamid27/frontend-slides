...
```js
fontFamily: {
  display: "'Archivo Black', 'Noto Serif SC', sans-serif",
...
- The `rule` design token in the inline tailwind.config (#0F0F0F, identical to `ink`) is defined but not used explicitly — it is reserved for horizontal rule elements and can be considered an alias for ink.
